...
foreach (Kitap ktp in _ktphane.kitap_listele()) { Console.WriteLine(ktp); }
...
{//kitap ->ekleme-listeleme-guncelleme-silme
    ...
    public void kitap_guncelle(Kitap orj_kitap, Kitap yeni_kitap)
    {
        ...
        if (aranan != -1) { kitaplar[aranan] = yeni_kitap; }
    }
    ...
    {
        foreach (Kitap s in kitaplar.ToList())
        {
            if(s.baslik.Contains(secilen_kitap))
            {
                kitap_sil(kitaplar.IndexOf(s));
            }
        }
    }
}
...
/*
 * Not edilme: 20.11.2024
 * get,set,enum sınavda var 
 */
